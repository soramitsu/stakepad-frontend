...
export function useRadiosRegistration({
  ...
  function registerRadio({
    ...
    const index = eagerComputed<number>(() =>
      elRef.value ? radioElements.value.indexOf(elRef.value) : -1,
    );

    const isItTheFirstRadioInTheGroup = eagerComputed<boolean>(
      () => index.value === 0,
    );

    const isChecked = eagerComputed<boolean>(
