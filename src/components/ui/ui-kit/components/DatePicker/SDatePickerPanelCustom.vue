...
const customInputValue = (field: string) => {
  return props.formatDate(
    props.stateStore.rangeState[field as keyof RangeState],
  );
};
...
const customInputValuePick = computed(() => {
  return props.formatDate(
    props.stateStore.pickState[props.stateStore.pickState.length - 1],
  );
});
...
function handleInput(event: Event) {
  ...
  const res = state.time
    ? dateTimeMask(event.target.value)
