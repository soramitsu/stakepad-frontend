  ...
  for (let i = 0; i < DAYS_IN_CALENDAR; i++) {
    ...
    if (state.type === "range") {
      ...
      if (rangeState.selecting) {
        ...
        const arr = [existingDate, hoveredDate].sort(
          (a, b) => a.getTime() - b.getTime(),
        );
        cell.inRange =
          time >= getDateTimestamp(arr[0]) && time <= getDateTimestamp(arr[1]);
      } else {
        if (rangeState.startDate && rangeState.endDate) {
          cell.inRange =
            time >= getDateTimestamp(rangeState.startDate) &&
            time <= getDateTimestamp(rangeState.endDate);
        }
      }
    ...
    if (state.type === "pick") {
      ...
      if (cell.start) {
        const timeOfDay = selectedDates.find(
          (item: Date) => getDateTimestamp(item) === getDateTimestamp(time),
        );
    ...
    if (i >= 0 && i <= 13) {
      ...
      if (index >= numberOfDaysFromPreviousMonth) {
        ...
        cell.text =
          ...
          (numberOfDaysFromPreviousMonth - (index % 7)) +
          1 +
          Math.floor(index / 7);
    ...
    cell.disabled = !state.dateFilter(
      new Date(year.value, cell.month, cell.text),
    );
...
<template>
  ...
  >
    ...
    >
      ...
      <span v-if="state.time" v-show="cell.time" class="absolute date-time">{{
        cell.time
      }}</span>
    </div>
  </div>
