...
import { type Ref, type BaseTransitionProps, ref } from "vue";

/**
 * TODO Reuse this functionality in `SModal` somehow (remove logic duplication)
 */
export function useWrappedTransitionVisibility({
  show,
  eager,
}: {
  show: Ref<boolean>;
  ...
  const wrapperIf = eagerComputed(
    () => eager.value || show.value || isContentMounted.value,
  );
  const wrapperShow = eagerComputed(
    () => !eager.value || isContentMounted.value || show.value,
  );
