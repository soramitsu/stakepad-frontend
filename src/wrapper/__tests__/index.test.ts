...
describe("ethersWrapper", async () => {
  ...
  const mockContract = ethersWrapper.createContract(
    "0x0",
    {},
    mockProvider,
  ) as any;
  ...
  it("transferContract", async () => {
    expect(
      ethersWrapper.transferContract(mockContract, mockContract, "100").hash,
    ).toEqual(
  ...
  it("sendTransaction", async () => {
    expect(
      await ethersWrapper.sendTransaction(mockSigner, "0x0", "0x0", "100"),
    ).toEqual({
      hash: "0x0",
    });
  });
  ...
  it("signMessage", async () => {
    expect(await ethersWrapper.signMessage(mockProvider, "message")).toEqual({
      message: "message",
    });
  });

  it("getGasNative", async () => {
    expect(await ethersWrapper.getGasNative(mockProvider, "100")).toEqual(
      "210000",
    );
  });
  ...
  it("getGasContract", async () => {
    expect(
      await ethersWrapper.getGasContract(mockProvider, mockContract),
    ).toEqual("441000000");
  });
});
